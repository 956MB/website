...
export default function ExtraRow({ entry }: { entry: IEntryGroup }) {
	return (
		<div className="flex flex-col flex-wrap w-full relative justify-center items-center">
			...
			<div className={clsx(
                "grid justify-center items-start gap-5 lg:gap-4 w-full max-w-screen-3xl sm:pb-7 pt-5 sm:pt-4",
                entry.category === "icons" ? "grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6" : "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3"
            )}>
				{React.Children.toArray(
					entry.items.map((item, i) => {
						return (
                            <RenderIfVisible defaultHeight={ITEM_DEF_HEIGHT}>
                                <div
                                    className={clsx(
                                ...
                                >
                                    ...
                                    <a
                                        className={clsx(
                                            ...
                                            item.link && (item.category === "wallpaper" && "child:hover:scale-105")
                                        )}
                                    ...
                                    >
                                        <Image
                                            ...
                                            className={clsx(
                                                ...
                                                item.category === "icon" ? "aspect-square object-contain scale-75" : "aspect-video sm:aspect-video object-cover"
                                            )}
                                    ...
                                    <div
                                        className={clsx(
                                            ...
                                            (item.summary && item.summary.length <= 0) && "h-[53px] min-h-[53px] max-h-[53px]"
                                        )}
                                    >
                                                ...
                                                <div className="flex flex-row gap-2 items-center justify-start w-full">
                                                    ...
                                                    {item.credit && (
                                                        <a className="p-[2px] bg-yellow-500/20 rounded-full">
                                                            <PiWarningCircleBold size={16} className="text-yellow-500 min-w-[16px] min-h-[16px]" />
                                                        </a>
                                                    )}
